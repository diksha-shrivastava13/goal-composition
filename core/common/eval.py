...
def evaluate_single(config, eval_env, env_params, rng, train_state):
    ...
    dr_ob_level = sample_0b_level(rng_reset)
    dr_25b_level = sample_25b_level(rng_reset)
    levels = Level.load_prefabs(config["eval_levels"])

    levels = jax.tree_map(lambda x, y: jnp.append(x, jnp.array([y]), axis=0), levels, dr_ob_level)
...
def log_eval_main(stats, train_state, config, level_sampler, env_vis, env, env_params, log_sampler_data=True, run_wandb_log=True):
    ...
    if config["lever_game"]:
        ...
        eval_level_names = config["eval_levels"] + ["db_0b", "dr_25b"]
    log_dict.update({f"solve_rate/{name}": solved.mean() for name, solved in zip(eval_level_names, all_solved)})
    ...
    if sampler is not None:
        ...
        if not config["lever_game"]:
            log_dict.update({"level_sampler/percentage_walls": ((sampler["levels"].wall_map * good[..., None, None]).sum(axis=-1).sum(axis=-1).sum() / good.sum())})
        if config["tmaze"]:
        ...
        def log_and_plot_single_sampler(sampler, num_images=config["num_images_to_plot"]):
            ...
            # we want the highest scoring levels first
            ...
            for i in range(num_images):
                score = weights_for_levels[top_levels_indices[i]]
                subscore = w_score[top_levels_indices[i]]
                substalesness = w_staleness[top_levels_indices[i]]
                subsub = scores_proper[top_levels_indices[i]]
                cap = f"{score:.2g} (rnk={subscore:.2g}); stl={substalesness:.2g}; sc={subsub:.2g})"
                buffer_images.append(wandb.Image(env_vis.get_frame(jax.tree_map(lambda x: x[i], init_env_state), env_params), caption=cap))
    ...
    if len(dr_inds) > 0:
        ...
        if config["lever_game"]: log_dict.update({"level_complexity/dr_num_visible_levers": stats["num_visible_levers"][dr_inds[-1]]})
    ...
    if len(replay_inds) > 0:
        ...
        if config["lever_game"]: log_dict.update({"level_complexity/replay_num_visible_levers": stats["num_visible_levers"][replay_inds[-1]]})
