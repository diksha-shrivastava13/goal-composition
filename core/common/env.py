...
def get_environment_and_params(config, return_general_kwargs=False):
    ...
    if config["tmaze"]:
        env_class = TMazeGame
        env_kws = {"reward_goal": config["tmaze_goal_reward"],
                   "reward_not_goal": config["tmaze_not_goal_reward"]}
    ...
    if config["lever_game"]:
        ...
        env_kws = {
            ...
            "reward_correct": config["level_game_reward_correct"],
            "reward_incorrect": config["level_game_reward_incorrect"],
            "multiplier_invisible": config["lever_game_multiplier_invisible"],
        }
        get_gen_func = make_lever_level_generator
        get_gen_kws = {"num_actions": config["lever_game_actions"],
                       "visible_answer_probability": config["lever_game_visible_answer_probability"]}

    wrappers = []
    if config["blindfold"]:
        wrappers.append(
            (BlindFoldWrapper, {"make_minus_one": config["blindfold_minus_one"],
                                "make_10": config["blindfold_ten"]},
             make_blind_level_sampler, {"blind_prob": config["blindfold_probability"],
                                        "impossible_prob": config["blindfold_impossible_probability"]})
        )
    ...
    env_vis = MazeVisualiser(env, tile_size=8)
