...
class BlindFoldWrapper(UnderspecifiedEnvWrapper):
    def __init__(self, env: UnderspecifiedEnv, make_minus_one=False, make_10=False) -> None:
        super().__init__(env)
    ...
    def reset_env_to_level(self, rng: PRNGKey, level: BlindFoldWrappedLevel, params: EnvParams) -> Tuple[Observation, EnvState]:
    ...
    def step_env(self, rng: PRNGKey, state: BlindFoldWrappedState, action: int | float, params) -> Tuple[chex.ArrayTree, EnvState, float, bool, dict]:
        obs, new_state, reward, done, info = self._env.step_env(rng, state.env_state, action, params)
        obs = self._format_obs(obs, state.is_blind)
        return obs, state.replace(_env_state=new_state), reward, done, info


def make_blind_level_sampler(original_sampler, blind_prob: float = 0.5, impossible_prob = 0.0):
    ...
    def sample_random(rng: chex.PRNGKey):
        ...
        prison = jax.tree_map(lambda x: x[0], OGMazeLevel.load_prefabs(["Labyrinth"]))
