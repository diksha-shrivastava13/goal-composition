...
TMAZE_LEVEL_RIGHT_ACTUAL_MAZE = Level(**to_state_dict(TMAZE_LEVEL_RIGHT), is_tmaze=True)
TMAZE_LEVEL_LEFT_ACTUAL_MAZE = Level(**to_state_dict(TMAZE_LEVEL_LEFT), is_tmaze=True)

class TMazeGame(Maze):
    """
    This environment is either a normal maze game or a T-maze. If it is a T-maze, then it has three states,
    with the goal being invisible. The episode terminates after one step.
    """
    ...
    def init_state_from_level(self, level: Level):
    ...
    def _step_agent(self, key: PRNGKey, state: EnvState, action: int, params: EnvParams) -> Tuple[EnvState, float]:
        ...
        # terminates after one step if we are in a t_maze
        new_state = jax.tree_map(
            lambda new, old: jax.lax.select(state.is_tmaze & has_moved, new, old),   # if t_maze and moved, terminate
            new_state.replace(terminal=True), new_state
        )

        # the reward depends on the t-maze
        reward_for_tmaze = jnp.where(reward > 0, self.reward_goal, jnp.where(has_moved, self.reward_not_goal, reward))
        reward = jnp.where(state.is_tmaze, reward_for_tmaze, reward)
...
def make_tmaze_level_generator(height: int, width: int, n_walls: int, tmaze_probability: float = 0.5):
    ...
    def sample(rng: chex.PRNGKey) -> Level:
        ...
        old_level = jax.tree_map(
            lambda old, left, right: jax.lax.select(is_tmaze, (jax.lax.select(
                tmaze_left_or_right, left, right)), old), old_level, t_left, t_right
        )
