...
import wandb
from core.common.arguments import setup_arguments
from core.common.env import get_environment_and_params
from core.common.plr import get_level_sampler, insert_to_level_sampler, update_level_sampler
from core.common.ppo import compute_gae, create_optimizer, sample_trajectories_rnn, update_actor_critic_rnn, \
    ActorCriticFeedForward
from core.common.eval import evaluate_single, log_eval_main, setup_checkpointing, setup_wandb, update_metrics
from jaxued.wrappers.autoreplay import AutoReplayWrapper
from core.wrappers.parallel_step import ParallelStepWrapper, ParallelStepLevel
...
def main(config=None):
    ...
    if config["lever_game"]:
        from core.common.ppo import ActorCriticFeedForward as ActorCritic
    else:
        from core.common.ppo import ActorCritic

    def log_eval(stats, train_state):
        # ReMiDi uses multi-buffer samplers (shape: num_outer_adversaries x ...),
        # which are incompatible with standard logging. Disable sampler logging for now.
    ...
    def train(rng, train_state):
        ...
        def on_replay_levels(rng, train_state, all_init_obs, all_init_states, mask_which_are_good,):
            ...
            # this does the rollouts on all the previous levels in the buffer
        ...
        def on_new_levels(rng, train_state, all_init_obs, all_init_states, mask_which_are_good,
            ...
            def make_single_level(level):
                return ParallelStepLevel(level=level, par_env_states=all_init_states, par_env_obs=all_init_obs,
                                         mask_of_levels_to_consider=mask_which_are_good)

            new_levels = jax.vmap(sample_random_level)(
                jax.random.split(rng_levels, n_sampl := config["num_train_envs"]))
            wrapped_levels = jax.vmap(make_single_level)(new_levels)

            init_obs, init_env_state = jax.vmap(wrapped_env.reset_to_level, in_axes=(0, 0, None))(
                jax.random.split(rng_reset, config["num_train_envs"]), wrapped_levels, env_params)
            (
                (rng, train_state, hstate, last_obs, last_env_state, last_value),
                (obs, actions, rewards, dones, log_probs, values, info),
            ) = sample_trajectories_rnn(
            ...
            sampler, scores = insert_to_level_sampler(config, env._env, env_params, level_sampler, sampler, new_levels,
                                                      values, advantages, dones, rewards, own_extras_to_add={
                    'init_obs': init_obs
        ...
        def train_step(carry, _):
            ...
            train_state = train_state.replace(
                current_number_of_updates_for_this_buffer=train_state.current_number_of_updates_for_this_buffer + 1)
            ...
            # Update the stats
            my_inner_sampler = inner_train_state.sampler
            ...
            AA, BB, CC = jax.tree_map(lambda old, new: jax.lax.select(
                train_state.current_number_of_updates_for_this_buffer > config['number_of_replays_per_buffer'], new, old),
                                      (train_state.current_buffer_index, train_state.current_number_of_updates_for_this_buffer,
                                       train_state.has_reached_mmr), (get_new_index(train_state.current_buffer_index,
                                                                                    config["num_outer_adversaries"])))
