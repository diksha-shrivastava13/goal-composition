...
from jaxued.environments.underspecified_env import EnvState, EnvParams, Observation, Level
...
class ParallelStepWrapper(UnderspecifiedEnv):
    ...
    def step_env(
        ...
        return (obs, state.replace(env_state=env_state,
                                  par_env_states=par_env_state,
                                  is_consistent_thus_far=state.is_consistent_thus_far & mask_to_use),
                reward, done, info)

    def reset_env_to_level(
        self, rng: chex.PRNGKey, level: ParallelStepLevel, params: EnvParams
    ) -> Tuple[Observation, EnvState]:

        obs, env_state = self._env.reset_to_level(rng, level.level, params)
        return obs, ParallelStepState(env_state=env_state,
                                      par_env_states=level.par_env_states,
