...
from jaxued.environments.maze.env import Observation as MazeObservation, DIR_TO_VEC
...
def compute_visibility_mask(
    ...
    """
    Compute visibility masks by running a diagnostic rollout on the given level.

    Runs a policy rollout and tracks which cells fall within the agent's view cone
    at each timestep. Returns three mutually exclusive masks partitioning the grid:
    - observed: cells that were within the agent's view at least once
    - adjacent: cells not observed but within 1-cell dilation of observed cells
    - distant: all remaining cells

    This runs OUTSIDE jit as a diagnostic tool (Python loop over steps).

    Args:
        env: Environment instance
        env_params: Environment parameters
        train_state: Current training state with policy
        level: Level to run rollout on
        num_envs: Number of parallel environments (typically 1)
        num_steps: Number of rollout steps
        agent_view_size: Size of agent's view (default 5)
        env_height: Grid height
        env_width: Grid width

    Returns:
        (observed_mask, adjacent_mask, distant_mask): boolean arrays of shape (env_height, env_width)
    """
    # Initialize visited mask
    visited = np.zeros((env_height, env_width), dtype=bool)

    # Reset env to given level
    rng = jax.random.PRNGKey(42)
    # Batch level for vmap
    batched_level = jax.tree_util.tree_map(lambda x: x[None, ...], level)
    rng, rng_reset = jax.random.split(rng)
    obs, env_state = jax.vmap(env.reset_to_level, in_axes=(0, 0, None))(
        jax.random.split(rng_reset, num_envs), batched_level, env_params
    )

    done = jnp.zeros(num_envs, dtype=bool)

    if hasattr(train_state, 'curriculum_state') and train_state.curriculum_state is not None:
        hstate = ActorCriticWithCurriculumPrediction.initialize_carry((num_envs,))
    else:
        hstate = ActorCritic.initialize_carry((num_envs,))

    half_view = agent_view_size // 2

    for step_i in range(num_steps):
        # Extract agent position and direction from env_state
        # env_state may be wrapped in AutoReplayState
        inner_state = env_state.env_state if hasattr(env_state, 'env_state') else env_state
        agent_pos = np.array(inner_state.agent_pos)  # (num_envs, 2) - [x, y]
        agent_dir = np.array(inner_state.agent_dir)   # (num_envs,)

        # Compute view bounding box for each env
        for e in range(num_envs):
            pos = agent_pos[e]  # [x, y]
            d = int(agent_dir[e])
            dir_vec = np.array(DIR_TO_VEC[d])  # [dx, dy]

            # Compute the forward bounds
            fwd_bound1 = pos
            fwd_bound2 = pos + dir_vec * (agent_view_size - 1)

            # Compute side bounds: perpendicular to direction
            # side_offset applies to axes where dir_vec is 0
            side_vec = np.array([1 if dir_vec[0] == 0 else 0,
                                 1 if dir_vec[1] == 0 else 0])
            side_bound1 = pos + side_vec * half_view
            side_bound2 = pos - side_vec * half_view

            # Get bounding box from all four corner points
            all_x = [fwd_bound1[0], fwd_bound2[0], side_bound1[0], side_bound2[0]]
            all_y = [fwd_bound1[1], fwd_bound2[1], side_bound1[1], side_bound2[1]]
            xmin = max(0, min(all_x))
            xmax = min(env_width - 1, max(all_x))
            ymin = max(0, min(all_y))
            ymax = min(env_height - 1, max(all_y))

            # Mark all cells in bounding box as visited
            visited[int(ymin):int(ymax)+1, int(xmin):int(xmax)+1] = True
    ...
    # Convert to jnp for mask operations
    observed_mask = jnp.array(visited)

    # Compute adjacent mask: 1-cell dilation of observed minus observed
    # Pad with False, then take OR of all 4 directional shifts
    padded = jnp.pad(observed_mask, 1, mode='constant', constant_values=False)
    dilated = (
        padded[1:-1, 1:-1] |  # center (original)
        padded[0:-2, 1:-1] |  # shift up
        padded[2:, 1:-1]   |  # shift down
        padded[1:-1, 0:-2] |  # shift left
        padded[1:-1, 2:]      # shift right
    )
    adjacent_mask = dilated & ~observed_mask

    # Distant mask: everything else
...
def compute_zone_decomposed_wall_metrics(
    ...
    - baseline_accuracy: majority-class baseline (max of wall_density, 1-wall_density)
...
def compute_goal_displacement_metrics(
    predictions,
    actual_level,
    sampler,
    level_sampler_module,
    env_height=DEFAULT_ENV_HEIGHT,
    env_width=DEFAULT_ENV_WIDTH,
):
    """
    Compute goal displacement from buffer distribution centroid.

    Tests whether prediction accuracy is just distributional memorization by
    correlating goal distance from the buffer centroid with prediction quality.

    All operations are JAX-compatible (runs inside jit).

    Args:
        predictions: Dict with 'goal_logits' of shape (env_height * env_width,)
        actual_level: Level with goal_pos [x, y]
        sampler: Level sampler state dict
        level_sampler_module: LevelSampler instance for get_levels
        env_height: Grid height
        env_width: Grid width

    Returns:
        Dict with goal displacement and prediction metrics
    """
    # Get all valid buffer goals
    buffer_size = sampler["size"]
    max_capacity = sampler["scores"].shape[0]
    valid_mask = (jnp.arange(max_capacity) < buffer_size).astype(jnp.float32)

    # Get all levels from buffer
    all_indices = jnp.arange(max_capacity)
    all_levels = level_sampler_module.get_levels(sampler, all_indices)

    # Compute buffer goal centroid (masked mean)
    goal_x = all_levels.goal_pos[:, 0].astype(jnp.float32)
    goal_y = all_levels.goal_pos[:, 1].astype(jnp.float32)
    safe_size = jnp.maximum(valid_mask.sum(), 1.0)
    centroid_x = (goal_x * valid_mask).sum() / safe_size
    centroid_y = (goal_y * valid_mask).sum() / safe_size

    # L2 distance from actual goal to centroid
    actual_x = actual_level.goal_pos[0].astype(jnp.float32)
    actual_y = actual_level.goal_pos[1].astype(jnp.float32)
    goal_distance_from_centroid = jnp.sqrt(
        (actual_x - centroid_x) ** 2 + (actual_y - centroid_y) ** 2
    )

    # Goal prediction accuracy: does argmax match actual?
    goal_idx = actual_level.goal_pos[1] * env_width + actual_level.goal_pos[0]
    predicted_idx = jnp.argmax(predictions['goal_logits'])
    goal_prediction_correct = (predicted_idx == goal_idx).astype(jnp.float32)

    # Probability assigned to actual goal position
    goal_probs = jax.nn.softmax(predictions['goal_logits'])
    goal_prob_at_actual = goal_probs[goal_idx]

    return {
        'goal_distance_from_centroid': goal_distance_from_centroid,
        'goal_prediction_correct': goal_prediction_correct,
        'goal_prob_at_actual': goal_prob_at_actual,
        'goal_centroid_x': centroid_x,
        'goal_centroid_y': centroid_y,
    }
...
def compute_visited_mask_from_positions(
    ...
    Uses the same view-cone geometry as the environment's partial observation.
    ...
    # This gives a reasonable approximation of the agent's view cone
...
def apply_curriculum_prediction_gradient(
    ...
    Args:
        ...
        env_height: Environment height
        env_width: Environment width
        wall_mask: Optional (H, W) boolean mask from previous trajectory visited cells
    ...
    def prediction_loss_fn(params):
        ...
        loss, metrics = compute_curriculum_prediction_loss(
            ...
            env_width=env_width,
            wall_mask=wall_mask,
            wall_loss_region=wall_loss_region,
        )
...
class TrainState(BaseTrainState):
    ...
    # Visited mask from previous trajectory for wall loss masking
    last_visited_mask: Optional[chex.Array] = struct.field(pytree_node=True, default=None)
...
def sample_trajectories_rnn(
    ...
    def sample_step(carry, _):
        ...
        # Track agent positions/directions for visited mask computation
        # env_state is AutoReplayState which wraps the inner EnvState
        traj_info = {
            'agent_pos': env_state.env_state.agent_pos,  # (N, 2)
            'agent_dir': env_state.env_state.agent_dir,   # (N,)
        }

        carry = (rng, train_state, hstate, next_obs, env_state, done)
        return carry, (obs, action, reward, done, log_prob, value, traj_info)
...
def main(config=None, project="JaxUED-minigrid-maze"):
        ...
        if config.get("use_curriculum_prediction", False) and "pred" in stats:
            ...
            for branch_id, branch_name in branch_names.items():
                ...
                if count > 0:
                    ...
                    # Zone evaluation losses (observed/adjacent/distant) - always logged
                    for zone in ["observed", "adjacent", "distant"]:
                        zone_key = f"zone_loss/{zone}"
                        zone_vals = pred_metrics.get(zone_key, jnp.array([0.0]))
                        if hasattr(zone_vals, 'shape') and zone_vals.shape:
                            zone_masked_mean = (zone_vals * mask).sum() / count
                        else:
                            zone_masked_mean = zone_vals
                        log_dict[f"curriculum_pred/{branch_name}/{zone_key}"] = float(zone_masked_mean)

                    # Zone membership for goal/agent positions
                    for zm_metric in ["goal_in_observed", "goal_in_adjacent", "goal_in_distant",
                                      "agent_in_observed", "agent_in_distant"]:
                        zm_key = f"zone_membership/{zm_metric}"
                        zm_vals = pred_metrics.get(zm_key, jnp.array([0.0]))
                        if hasattr(zm_vals, 'shape') and zm_vals.shape:
                            zm_masked_mean = (zm_vals * mask).sum() / count
                        else:
                            zm_masked_mean = zm_vals
                        log_dict[f"curriculum_pred/{branch_name}/{zm_key}"] = float(zm_masked_mean)

                    # Zone coverage fractions
                    for zc_metric in ["observed_frac", "adjacent_frac", "distant_frac"]:
                        zc_key = f"zone_coverage/{zc_metric}"
                        zc_vals = pred_metrics.get(zc_key, jnp.array([0.0]))
                        if hasattr(zc_vals, 'shape') and zc_vals.shape:
                            zc_masked_mean = (zc_vals * mask).sum() / count
                        else:
        ...
        if config.get("use_curriculum_prediction", False) and train_state is not None and train_state.curriculum_state is not None:
            ...
            if update_count > 0 and update_count % pred_eval_freq == 0:
                try:
                    ...
                    # Experiment II: Three-zone wall prediction decomposition
                    try:
                        observed_mask, adjacent_mask, distant_mask = compute_visibility_mask(
                            env, env_params, train_state, representative_level,
                            num_envs=1, num_steps=256,
                            agent_view_size=5,
                            env_height=env_max_height, env_width=env_max_width,
                        )
                        zone_metrics = compute_zone_decomposed_wall_metrics(
                            predictions, representative_level,
                            observed_mask, adjacent_mask, distant_mask,
                            env_height=env_max_height, env_width=env_max_width,
                        )
                        ...
                        print(f"Warning: Failed to compute zone decomposition: {e}")
    ...
    def create_train_state(rng) -> TrainState:
        ...
        return TrainState.create(
            ...
            last_visited_mask=jnp.ones((env_max_height, env_max_width), dtype=jnp.bool_) if config.get("use_curriculum_prediction", False) else None,
        )
    ...
    def train_step(carry: Tuple[chex.PRNGKey, TrainState], _):
        ...
        def on_new_levels(rng: chex.PRNGKey, train_state: TrainState):
            ...
            # Compute visited mask from trajectory if wall loss masking is enabled
            if config.get("use_curriculum_prediction", False):
                observed, _, _ = compute_visited_mask_from_positions(
                    info['agent_pos'], info['agent_dir'],
                    agent_view_size=config["agent_view_size"],
                    env_height=env_max_height, env_width=env_max_width,
                )
                train_state = train_state.replace(last_visited_mask=observed)
        ...
        # =====================================================================
        # SAVE PREVIOUS BATCHES (before branch overwrites them)
        # =====================================================================
        prev_dr_batch = train_state.dr_last_level_batch
        prev_replay_batch = train_state.replay_last_level_batch
        prev_mutation_batch = train_state.mutation_last_level_batch
        prev_visited_mask = train_state.last_visited_mask  # None if masking disabled
        ...
        if config.get("use_curriculum_prediction", False) and train_state.curriculum_state is not None:
            ...
            if curriculum_features is not None:
                ...
                # Experiment III: Goal displacement metrics (JAX-compatible, runs inside jit)
                goal_disp_metrics = compute_goal_displacement_metrics(
                    curriculum_predictions,
                    representative_level,
                    sampler,
                    level_sampler,
                ...
                metrics["pred"].update(goal_disp_metrics)

                # Experiment III: Comprehensive batch displacement metrics
                # Select the previous batch for the active branch
                prev_batch = jax.lax.switch(branch, [
                    lambda: prev_dr_batch,
                    lambda: prev_replay_batch,
                    lambda: prev_mutation_batch,
                ])
                displacement_metrics = compute_batch_displacement_metrics(
                    level_batch, prev_batch,
                    env_height=env_max_height, env_width=env_max_width,
                )
                metrics["pred"].update(displacement_metrics)
...
if __name__ == "__main__":
    ...
    curriculum_group.add_argument("--wall_loss_region", type=str, default="full",
                                   choices=["full", "explored", "frontier"],
                                   help="Training region for wall prediction loss. "
                                        "'full': all cells (default, original behavior), "
                                        "'explored': only cells visited in previous trajectory, "
                                        "'frontier': explored + adjacent bubble around trajectory. "
                                        "Zone evaluation (observed/adjacent/distant) is always logged.")
