...
\textbf{Protocol}: Collect (level\_features, prediction\_loss) pairs during training. Use PySR (symbolic regression) to extract interpretable formulas: $\Uhat = f(\text{level\_features})$, where $-\text{prediction\_loss}$ approximates the log-probability the model assigns to each level. For \paired{}, additionally fit $\text{regret} = g(\text{level\_features})$.
