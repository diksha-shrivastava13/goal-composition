...
class AdversaryActorCritic(nn.Module):
    ...
    def __call__(self, inputs: Tuple[Observation, chex.Array], hidden):
        ...
        time_value = nn.Embed(self.max_timesteps + 1, 10, name="time_embed", embedding_init=orthogonal(1.0))(
...
def main(config=None, project="JAXUED_TEST"):
    ...
    def log_eval(stats):
        ...
        log_dict.update({f"images/levels": [wandb.Image(np.array(image), caption=make_caption(i)) for i, image in
                                            enumerate(stats["levels"])]})
    ...
    adv_env = MazeEditor(env, random_z_dimensions=config['adv_random_z_dimension'],
    ...
    def create_train_state(rng):
        def create_inner_train_state(rng, env, env_params, network_cls, prefix, network_kws={}):
            ...
            obs = jax.tree_util.tree_map(
                lambda x: jnp.repeat(jnp.repeat(x[None, ...], config["num_train_envs"], axis=0)[None, ...], 256,
                                     axis=0),
                obs,
            )
        ...
        return TrainState(
            ...
            adv_train_state=create_inner_train_state(rng_adv, adv_env, adv_env_params, AdversaryActorCritic, "adv_",
    ...
    def train_step(carry, _):
        def rollout(rng, env, env_params, train_state, init_hstate, levels, num_steps, prefix):
            ...
            advantages, targets = compute_gae(config[f"{prefix}gamma"], config[f"{prefix}gae_lambda"], last_value,
                                              values, rewards, dones)
            return (obs, actions, dones, log_probs, values, targets, advantages), (dones, rewards, last_env_state,
                                                                                   last_value, info)
        ...
        rng, _rng = jax.random.split(rng)
        empty_levels = jax.tree_util.tree_map(lambda x: jnp.array([x]).repeat(config["num_train_envs"], axis=0),
                                              sample_empty_level())
        adv_rollout, (_, _, last_env_state, last_value, _) = rollout(_rng, adv_env, adv_env_params, adv_train_state,
                                                                     AdversaryActorCritic.initialize_carry(
                                                                         (config["num_train_envs"],)), empty_levels,
                                                                     config["adv_num_steps"], "adv_")
        levels = last_env_state.level

        # protagonist rollout
        rng, _rng = jax.random.split(rng)
        pro_rollout, (dones, rewards, _, _, _) = rollout(_rng, env, env_params, pro_train_state,
                                                         ActorCritic.initialize_carry((config["num_train_envs"],)),
                                                         levels, config["student_num_steps"], "student_")
        pro_mean_returns, pro_max_returns, pro_eps = compute_max_mean_returns_epcount(dones, rewards)

        # antagonist rollout
        rng, _rng = jax.random.split(rng)
        ant_rollout, (dones, rewards, _, _, _) = rollout(_rng, env, env_params, ant_train_state,
                                                         ActorCritic.initialize_carry((config["num_train_envs"],)),
                                                         levels, config["student_num_steps"], "student_")
        ...
        advantages, targets = compute_gae(config["adv_gamma"], config["adv_gae_lambda"], last_value, values, rewards,
                                          dones)
        adv_rollout = (obs, actions, dones.at[-1].set(True), log_probs, values, targets, advantages)

        (rng, pro_train_state), pro_losses = update(rng, pro_train_state,
                                                    ActorCritic.initialize_carry((config["num_train_envs"],)),
                                                    pro_rollout, "student_")
        (rng, ant_train_state), ant_losses = update(rng, ant_train_state,
                                                    ActorCritic.initialize_carry((config["num_train_envs"],)),
                                                    ant_rollout, "student_")
        (rng, adv_train_state), adv_losses = update(rng, adv_train_state,
    ...
    def train_and_eval_step(runner_state, _):
        ...
        states, cum_rewards, episode_lengths = jax.vmap(eval, (0, None))(
            jax.random.split(rng_eval, config["eval_num_attempts"]), train_state.pro_train_state)
